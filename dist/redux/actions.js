...
actions.createPage = (page, plugin, traceId) => {
  ...
  const result = _joi2.default.validate(internalPage, joiSchemas.pageSchema);
  if (result.error) {
...
actions.createLayout = (layout, plugin, traceId) => {
  ...
  let componentWrapperPath = joinPath(store.getState().program.directory, `.cache`, `layouts`, `${id}.js`);
  ...
  const result = _joi2.default.validate(internalLayout, joiSchemas.layoutSchema);
...
actions.createRedirect = (_ref) => {
  let {
    ...
    toPath
      ...
      rest = (0, _objectWithoutProperties3.default)(_ref, ["fromPath", "isPermanent", "redirectInBrowser", "toPath"]);
