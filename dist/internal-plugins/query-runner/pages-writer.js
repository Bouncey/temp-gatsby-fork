...
const writePages = async () => {
  ...
  const pagesData = pages.reduce((mem, { path, matchPath, componentChunkName, layout, jsonName }) => {
  ...
  await fs.writeFile((0, _path.joinPath)(program.directory, `.cache/pages.json`), JSON.stringify(pagesData, null, 4));
...
\n\n`;
  ...
  syncRequires += `exports.layouts = {\n${pageLayouts.map(l => `  "${l.componentChunkName}": preferDefault(require("${l.componentWrapperPath}"))`).join(`,\n`)}
}`;

  await fs.writeFile(`${program.directory}/.cache/sync-requires.js`, syncRequires);
...
\n`;
  ...
  asyncRequires += `exports.layouts = {\n${pageLayouts.map(l => `  "${l.componentChunkName}": require("gatsby-module-loader?name=${l.componentChunkName}!${l.componentWrapperPath}")`).join(`,\n`)}
}`;

  await fs.writeFile((0, _path.joinPath)(program.directory, `.cache/async-requires.js`), asyncRequires);
